...
(function () {
  ...
  /* -----------------------------------------------------------------------
     0. BROWSER NAVIGATION (back / forward)
  ----------------------------------------------------------------------- */
  on($('#nav-back'),    'click', function () { history.back(); });
  on($('#nav-forward'), 'click', function () { history.forward(); });
  ...
  if (markdownBody && tocNav && tocEnabled) {
    ...
    if (headings.length === 0) {
      ...
      headings.forEach(function (h, i) { headingMap.set(h, tocLinks[i]); });
